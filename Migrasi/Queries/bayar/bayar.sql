...
LEFT JOIN [dataawal].`master_attribute_petugas_baca` pb ON pb.`petugasbaca`=TRIM(SUBSTRING_INDEX(p.pembacameter, '(', 1)) AND pb.`idpdam`=@idpdam
LEFT JOIN [dataawal].`master_attribute_kelainan` kl ON kl.`kelainan`=p.kelainan
