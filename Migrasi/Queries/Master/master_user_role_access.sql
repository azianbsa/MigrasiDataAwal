...
AND idrole IN (SELECT idrole FROM master_user_role WHERE idpdam = @idpdam AND flaghapus = 0)
) a
JOIN 
(SELECT idrole,namarole FROM master_user_role WHERE idpdam = @idpdam AND flaghapus = 0) b
