...
JOIN `kotaparepare_dataawal`.`tampung_master_pelanggan_air` pe ON p.nosamb=pe.`nosamb`
...
LEFT JOIN `ba_bukasegel` ba ON ba.`nomorpermohonan`=p.`nomor` AND ba.`flaghapus`=0
